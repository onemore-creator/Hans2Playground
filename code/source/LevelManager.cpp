...
{
	return isLevelBuilding;
}


void LevelManager::BuildCodeChecker()
{
	if(isLevelBuilding) return;
	static int index = 0;

	if(InputManager::GetButtonDown(0, buildModeEnterCode[index]))
	{
		index++;
	}
	else if(InputManager::GetAnyButtonDown(0))
	{
		index = 0;
	}

	if(index >= buildModeEnterCode.size())
	{
		EnableLevelBuilder();
	}
}