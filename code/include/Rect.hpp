...
struct Rect : CollidableEntity {
  ...
  // Exposed
  float fallSpeed = 0.0f; // current downward velocity (px/s). Positive = falling, negative = climbing.
  float angle     = 0.0f; // nose angle in radians. 0 = level, positive = nose down, negative = nose up.
...
private:
  ...
  float pitchImpulse = -150.0f; // instant upward burst on A/B           (px / s)
